...
int (*x_recvmmsg)(int sockfd, struct mmsghdr *msgvec, unsigned int vlen,
                  int flags, struct timespec *timeout);

int (*x_sendmmsg)(int sockfd, struct mmsghdr *msgvec, unsigned int vlen,
                  int flags);

static int my_recvmmsg(int sockfd, struct mmsghdr *msgvec, unsigned int vlen,
                       int flags, struct timespec *timeout) {
  ...
  for (uint i = 0; i < vlen; ++i) {
    ...
    if (wait_for_one) flags |= MSG_DONTWAIT;
  }

  return nrecv ?: -1;
}

static int my_sendmmsg(int sockfd, struct mmsghdr *msgvec, unsigned int vlen,
                       int flags) {
  unlikely_if(vlen <= 0) {
    errno = EINVAL;
    return -1;
...
static inline void set_ipv6_only(int sockfd, int value) {
  unlikely_if(
    ...
    log_error("setsockopt(%d, IPV6_V6ONLY, %d): (%d) %s", sockfd, value, errno,
              strerror(errno));
    exit(errno);
  }
}
...
static inline void set_reuse_addr(int sockfd) {
  unlikely_if(
    ...
    log_error("setsockopt(%d, SO_REUSEADDR): (%d) %s", sockfd, errno,
              strerror(errno));
    exit(errno);
  }
}
...
void set_reuse_port(int sockfd) {
  unlikely_if(
    ...
    log_error("setsockopt(%d, SO_REUSEPORT): (%d) %s", sockfd, errno,
              strerror(errno));
    exit(errno);
  }
}
...
int new_udp_socket(int family, bool for_bind) {
  int sockfd =
      socket(family, SOCK_DGRAM | SOCK_NONBLOCK, 0); /* since Linux 2.6.27 */
  unlikely_if(sockfd < 0) {
    log_error("failed to create udp%c socket: (%d) %s",
              family == AF_INET ? '4' : '6', errno, strerror(errno));
    exit(errno);
  }
  if (for_bind) {
    if (family == AF_INET6)
      set_ipv6_only(sockfd,
                    0); /* allow msg from ipv4 when binding `::` addresses */
...
void skaddr_build(int family, union skaddr *noalias skaddr,
                  const char *noalias ipstr, u16 portno) {
  if (family == AF_INET) {
...
void skaddr_parse(const union skaddr *noalias skaddr, char *noalias ipstr,
                  u16 *noalias portno) {
