...
#define nlmsg_data(nlmsg) NLMSG_DATA(nlmsg)

/* end address of data */
#define nlmsg_dataend(nlmsg) ((void *)(nlmsg) + (nlmsg)->nlmsg_len)

static inline void *nlmsg_add_data(struct nlmsghdr *noalias nlmsg,
...
#define nla_len_calc(datalen) (NLA_HDRLEN + (datalen))

#define nla_size_calc(datalen) (NLA_HDRLEN + NLA_ALIGN(datalen))

#define nla_data(nla) ((void *)(nla) + NLA_HDRLEN)

static inline struct nlattr *nlmsg_add_nla(struct nlmsghdr *noalias nlmsg,
                                           u16 attrtype,
                                           const void *noalias data,
                                           size_t datalen) {
