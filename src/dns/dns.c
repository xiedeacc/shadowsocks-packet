...
static bool decode_name(char *noalias out, const char *noalias src, int len) {
  ...
  for (int first = 1; len >= 2;) {
    ...
    unlikely_if(label_len > len) {
      log_error("label length is greater than remaining length: %d > %d",
                label_len, len);
      return false;
    }
    src += label_len;
...
/* check dns packet */
static bool check_packet(bool is_query, const void *noalias packet_buf,
                         ssize_t packet_len, char *noalias name_buf,
                         int *noalias p_namelen) {
  /* check packet length */
  ...
  unlikely_if(header->qr != (is_query ? DNS_QR_QUERY : DNS_QR_REPLY)) {
    log_error("this is a %s packet, but header->qr is %u",
              is_query ? "query" : "reply", (uint)header->qr);
    return false;
  }
  ...
  unlikely_if(ntohs(header->question_count) != 1) {
    log_error("there should be one and only one question section: %u",
              (uint)ntohs(header->question_count));
    return false;
  }
  ...
  if (name_buf) {
    unlikely_if(!decode_name(name_buf, packet_buf, namelen)) return false;
  }
  if (p_namelen) *p_namelen = namelen;
  ...
  unlikely_if(packet_len < (ssize_t)sizeof(struct dns_query)) {
    log_error("remaining length is less than sizeof(dns_query): %zd < %zu",
              packet_len, sizeof(struct dns_query));
    return false;
  }
  ...
  unlikely_if(ntohs(query_ptr->qclass) != DNS_CLASS_INTERNET) {
    log_error("only supports standard internet query class: %u",
              (uint)ntohs(query_ptr->qclass));
    return false;
  }
...
static bool skip_name(const void *noalias *noalias p_ptr,
                      ssize_t *noalias p_len) {
  ...
  unlikely_if(len < (ssize_t)sizeof(struct dns_record)) {
    log_error("remaining length is less than sizeof(dns_record): %zd < %zu",
              len, sizeof(struct dns_record));
    return false;
  }
...
static bool foreach_ip(const void *noalias packet_buf, ssize_t packet_len,
                       int namelen,
                       bool (*f)(const void *noalias ip, bool v4, void *ud),
                       void *ud) {
  ...
  for (u16 i = 0; i < answer_count; ++i) {
    ...
    unlikely_if(packet_len < recordlen) {
      log_error("remaining length is less than sizeof(record): %zd < %zd",
    ...
    switch (ntohs(record->rtype)) {
      case DNS_RECORD_TYPE_A:
        unlikely_if(rdatalen != IPV4_BINADDR_LEN) {
          log_error("rdatalen is not equal to sizeof(ipv4): %u != %d",
                    (uint)rdatalen, IPV4_BINADDR_LEN);
          return false;
        }
      ...
      case DNS_RECORD_TYPE_AAAA:
        unlikely_if(rdatalen != IPV6_BINADDR_LEN) {
          log_error("rdatalen is not equal to sizeof(ipv6): %u != %d",
                    (uint)rdatalen, IPV6_BINADDR_LEN);
          return false;
        }
...
int dns_test_ip(const void *noalias packet_buf, ssize_t packet_len,
  ...
  unlikely_if(!foreach_ip(packet_buf, packet_len, namelen, test_ip,
                          &res)) return DNS_IPCHK_BAD_PACKET;
  return res;
}
