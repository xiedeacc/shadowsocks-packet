...
// DEFINE_string(config_path, "/etc/shadowsocks/config.json", "config");
...
static inline const char *filter_aaaa_by_tag(u8 name_tag) {
  if (is_filter_all_v6(g_noaaaa_query)) return "all";
...
static inline void reply_with_no_answer(const union skaddr *noalias addr,
                                        socklen_t addrlen, void *noalias query,
                                        size_t querylen) {
  struct dns_header *header = query;
  header->qr = DNS_QR_REPLY;
  header->rcode = DNS_RCODE_NOERROR;
  unlikely_if(sendto(s_bind_sockfd, query, querylen, 0, &addr->sa, addrlen) <
              0) {
    u16 port = 0;
    skaddr_parse(addr, s_ipstr_buf, &port);
    log_error("failed to send reply to %s#%u: (%d) %s", s_ipstr_buf, (uint)port,
              errno, strerror(errno));
  }
}

static void update_upstream_sock(int now) {
  likely_if(s_sock_create_time &&
            now - s_sock_create_time < SOCK_LIFETIME) return;

  if (s_sock_create_time)
    log_verbose("create new socket, old socket is used for %d seconds",
                now - s_sock_create_time);

  if (MYHASH_CNT(s_context_list) > 0U) {
    log_verbose("there are %u unfinished queries, continue to use the old",
                MYHASH_CNT(s_context_list));
  ...
  for (int i = 0; i <= SERVER_MAXIDX; ++i) {
    ...
    if (s_upstream_sockfds[i] >= 0)
      close(s_upstream_sockfds[i]); /* fd will be auto removed from the
                                       interest-list when it is closed */

    s_upstream_sockfds[i] =
        new_udp_socket(skaddr_family(&g_upstream_skaddrs[i]), false);
    ...
    unlikely_if(
        epoll_ctl(s_epollfd, EPOLL_CTL_ADD, s_upstream_sockfds[i], &ev)) {
      log_error("failed to register epoll event: (%d) %s", errno,
                strerror(errno));
      exit(errno);
    }
  }
...
static void handle_local_packet(void) {
  unlikely_if(MYHASH_CNT(s_context_list) >=
              65536U) { /* range:0~65535, count:65536 */
  ...
  ssize_t packet_len = recvfrom(s_bind_sockfd, s_packet_buf, PACKET_BUFSZ, 0,
                                &source_addr.sa, &source_addrlen);

  if (packet_len < 0) {
    unlikely_if(errno != EAGAIN && errno != EWOULDBLOCK) log_error(
        "failed to recv from bind socket: (%d) %s", errno, strerror(errno));
    return;
  }
  ...
  int namelen = 0;
  unlikely_if(
      !dns_check_query(s_packet_buf, packet_len, name_buf, &namelen)) return;
  ...
  u8 name_tag = (ascii_namelen > 0 && g_dnl_nitems)
                    ? get_name_tag(s_name_buf, ascii_namelen)
                    : g_default_tag;

  if_verbose {
    u16 port = 0;
    skaddr_parse(&source_addr, s_ipstr_buf, &port);
    log_info("query [%s] from %s#%u (%u)", s_name_buf, s_ipstr_buf, (uint)port,
             (uint)s_unique_msgid);
  }

  if (g_noaaaa_query & (NOAAAA_TAG_GFW | NOAAAA_TAG_CHN | NOAAAA_TAG_NONE) &&
      qtype == DNS_RECORD_TYPE_AAAA) {
    const char *rule = filter_aaaa_by_tag(name_tag);
    if (rule) {
      log_verbose("filter [%s] AAAA query, rule: %s", s_name_buf, rule);
      reply_with_no_answer(&source_addr, source_addrlen, s_packet_buf,
                           packet_len);
      return;
    }
  }
  ...
  for (int i = 0; i <= SERVER_MAXIDX; ++i) {
    ...
    if (is_chinadns_idx(i)) {
      ...
      if (g_noaaaa_query & NOAAAA_CHINA_DNS && qtype == DNS_RECORD_TYPE_AAAA)
        continue;
    } else {
      if (name_tag == NAME_TAG_CHN) continue;
      if (g_noaaaa_query & NOAAAA_TRUST_DNS && qtype == DNS_RECORD_TYPE_AAAA)
        continue;
    ...
    unlikely_if(n_sent != msg_n) {
      if (n_sent < 0)
        log_error("failed to send query to %s: (%d) %s", g_upstream_addrs[i],
                  errno, strerror(errno));
      else
        log_warning(
            "send query to %s: %d != %u (kernel buffer may not be enough)",
            g_upstream_addrs[i], n_sent, (uint)msg_n);
    }
  }
  ...
  if (!sent) { /* caused by no aaaa query (china or trust) */
    ...
    log_verbose("filter [%s] AAAA query, rule: %s", s_name_buf,
                (g_noaaaa_query & NOAAAA_CHINA_DNS) ? "chinadns" : "trustdns");
    reply_with_no_answer(&source_addr, source_addrlen, s_packet_buf,
                         packet_len);
    return;
  }
  ...
  MYHASH_ADD(s_context_list, context, &context->unique_msgid,
             sizeof(context->unique_msgid));
}

static inline void remove_answer(void *noalias packet_buf,
                                 ssize_t *noalias packet_len, int namelen) {
...
static inline bool use_china_reply(void *noalias packet_buf,
                                   ssize_t *noalias packet_len, int namelen) {
  ...
  bool only_chinadns =
      g_noaaaa_query & NOAAAA_TRUST_DNS && qtype == DNS_RECORD_TYPE_AAAA;
  if (only_chinadns && !(g_noaaaa_query & NOAAAA_CHINA_IPCHK)) return true;
  ...
  switch (dns_test_ip(packet_buf, *packet_len, namelen)) {
    ...
    case DNS_IPCHK_NOT_FOUND:
      ...
      log_verbose("no ip found in reply, see as %s",
                  g_noip_as_chnip ? "chnip (accept)" : "non-chnip (drop)");
...
static inline bool use_trust_reply(void *noalias packet_buf,
                                   ssize_t *noalias packet_len, int namelen) {
  u16 qtype = dns_qtype(packet_buf, namelen);

  bool only_trustdns =
      g_noaaaa_query & NOAAAA_CHINA_DNS && qtype == DNS_RECORD_TYPE_AAAA;
  if (!only_trustdns) return false; /* waiting for chinadns return */

  if (g_noaaaa_query & NOAAAA_TRUST_IPCHK &&
      dns_test_ip(packet_buf, *packet_len, namelen) == DNS_IPCHK_NOT_CHNIP) {
    log_verbose("answer ip is not china ip, change to no-answer (AAAA)");
    remove_answer(packet_buf, packet_len, namelen);
  }
  return true;
...
static void handle_remote_packet(int index) {
  ...
  ssize_t packet_len =
      recvfrom(sockfd, s_packet_buf, PACKET_BUFSZ, 0, NULL, NULL);

  if (packet_len < 0) {
    unlikely_if(errno != EAGAIN && errno != EWOULDBLOCK) log_error(
        "failed to recv from %s: (%d) %s", addr, errno, strerror(errno));
    return;
  }
  ...
  int namelen = 0;
  unlikely_if(
      !dns_check_reply(s_packet_buf, packet_len, name_buf, &namelen)) return;

  struct queryctx *context = NULL;
  struct dns_header *dns_header = s_packet_buf;
  MYHASH_GET(s_context_list, context, &dns_header->id, sizeof(dns_header->id));
  if (!context) {
    log_verbose("reply [%s] from %s (%u), result: ignore", s_name_buf, addr,
                (uint)dns_header->id);
    return;
  }
  ...
  if (is_chinadns_idx(index)) {
    if (context->name_tag == NAME_TAG_CHN ||
        use_china_reply(reply_buffer, &reply_length, namelen)) {
      log_verbose("reply [%s] from %s (%u), result: accept", s_name_buf, addr,
                  (uint)dns_header->id);
      if (context->trustdns_buf)
        log_verbose("reply [%s] from <previous-trustdns> (%u), result: filter",
                    s_name_buf, (uint)dns_header->id);
      if (g_add_tagchn_ip && context->name_tag == NAME_TAG_CHN) {
        log_verbose("add the answer ip of name-tag:chn [%s] to ipset",
                    s_name_buf);
        dns_add_ip(reply_buffer, reply_length, namelen, true);
      }
    } else {
      log_verbose("reply [%s] from %s (%u), result: filter", s_name_buf, addr,
                  (uint)dns_header->id);
      if (context->trustdns_buf) { /* trustdns returns before chinadns */
        log_verbose("reply [%s] from <previous-trustdns> (%u), result: accept",
                    s_name_buf, (uint)dns_header->id);
    ...
    if (context->name_tag == NAME_TAG_GFW || context->chinadns_got ||
        use_trust_reply(reply_buffer, &reply_length, namelen)) {
      log_verbose("reply [%s] from %s (%u), result: accept", s_name_buf, addr,
                  (uint)dns_header->id);
      if (g_add_taggfw_ip && context->name_tag == NAME_TAG_GFW) {
        log_verbose("add the answer ip of name-tag:gfw [%s] to ipset",
                    s_name_buf);
      ...
      if (!context->trustdns_buf) {
        log_verbose("reply [%s] from %s (%u), result: delay", s_name_buf, addr,
                    (uint)dns_header->id);
        context->trustdns_buf =
            malloc(sizeof(*context->trustdns_buf) + packet_len);
  ...
  unlikely_if(sendto(s_bind_sockfd, reply_buffer, reply_length, 0,
                     &context->source_addr.sa, source_addrlen) < 0) {
    u16 port = 0;
...
static void handle_timeout_event(struct queryctx *context) {
  log_verbose("upstream reply timeout, unique msgid: %u",
              (uint)context->unique_msgid);
  free_context(context);
}
...
int main(int argc, char *argv[]) {
  ...
  bool need_ipset =
      g_add_tagchn_ip || g_add_taggfw_ip || g_default_tag == NAME_TAG_NONE;
  ...
  if (g_repeat_times > 1)
    log_info("enable repeat mode, times: %u", (uint)g_repeat_times);

  log_info("%s no-ip reply from chinadns",
           g_noip_as_chnip ? "accept" : "filter");
  ...
  unlikely_if(
      bind(s_bind_sockfd, &g_bind_skaddr.sa, skaddr_size(&g_bind_skaddr))) {
    log_error("failed to bind address to socket: (%d) %s", errno,
              strerror(errno));
    return errno;
  }
  ...
  for (;;) {
    int event_count =
        retry_EINTR(epoll_wait(s_epollfd, events, MAX_EVENTS, timeout_ms));

    unlikely_if(event_count < 0) log_error(
        "epoll_wait() reported an error: (%d) %s", errno, strerror(errno));
    ...
    for (int i = 0; i < event_count; ++i) {
      ...
      unlikely_if(ev & EPOLLERR) {
        ...
        switch (data) {
          case CHINADNS1_IDX:
            log_error("upstream socket error %s: (%d) %s",
                      g_upstream_addrs[CHINADNS1_IDX], errno, strerror(errno));
            break;
          case CHINADNS2_IDX:
            log_error("upstream socket error %s: (%d) %s",
                      g_upstream_addrs[CHINADNS2_IDX], errno, strerror(errno));
            break;
          case TRUSTDNS1_IDX:
            log_error("upstream socket error %s: (%d) %s",
                      g_upstream_addrs[TRUSTDNS1_IDX], errno, strerror(errno));
            break;
          case TRUSTDNS2_IDX:
            log_error("upstream socket error %s: (%d) %s",
                      g_upstream_addrs[TRUSTDNS2_IDX], errno, strerror(errno));
            break;
          case BINDSOCK_MARK:
