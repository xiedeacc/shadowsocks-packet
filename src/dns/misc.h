...
/*
  void f(int n, int *noalias p, int *noalias q, const int *noalias a, const int
  *noalias b) { for (int i = 0; i < n; ++i) { p[i] = a[i] + b[i] q[i] = a[i] *
  b[i]
    }
  }
  ...
  ==================================================
  strict aliasing: an object can only be aliased by "compatible type" or "char
  type"
*/
...
/* to avoid breaking the constant properties of input parameters, do not use
 * __auto_type or __typeof__ */
...
#define bitvec_set1(vec, i) ((vec)[(ullong)(i) >> 3] |= (1U << ((ullong)(i)&7)))

#define bitvec_set0(vec, i) \
  ((vec)[(ullong)(i) >> 3] &= ~(1U << ((ullong)(i)&7)))

/* align to `n` (struct, struct member) */
#define struct_alignto(n) __attribute__((packed, aligned(n)))

/* dns packet max size (in bytes) */
#define DNS_PACKET_MAXSIZE 1472 /* compatible with edns */
#define DNS_PACKET_MINSIZE \
  (sizeof(struct dns_header) + DNS_NAME_ENC_MINLEN + sizeof(struct dns_query))
