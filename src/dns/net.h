...
extern int (*x_recvmmsg)(int sockfd, struct mmsghdr *msgvec, unsigned int vlen,
                         int flags, struct timespec *timeout);

/* compatible with old kernel (runtime) */
extern int (*x_sendmmsg)(int sockfd, struct mmsghdr *msgvec, unsigned int vlen,
                         int flags);
...
#define sendall(f, fd, base, len, args...)                           \
  ({                                                                 \
    ...
    do {                                                             \
      __auto_type ret_ =                                             \
          retry_EINTR(f(fd, &base_[nsent_], len_ - nsent_, ##args)); \
